...
namespace TouchHandle.myTool
{
    ...
    {
        #region 内部成员
        private static string FilePath = System.IO.Path.GetDirectoryName(System.Diagnostics.Process.GetCurrentProcess().MainModule.FileName) + "\\log\\" + DateTime.Now.ToString("yyyy.MM.dd") + ".txt";
        ...
        /// i will pu int log in the file with the error code Position
        /// </summary>
        /// <param name="yourException">your Exception</param>
        public static void PutInLogEx(Exception yourException)
        {
            string errorMessage = "StackTrace: " + yourException.StackTrace + "\r\n" + "Message: " + yourException.Message;
            if (isUsing == false)
            {
                isUsing = true;
                if (!File.Exists(FilePath))
